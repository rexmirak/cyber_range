...
def scenario():
    return {
        "metadata": {
            "name": "Provision Lab", "version": "1.0.0",
        ...
        "hosts": [
            {
                ...
                "base_image": "mysql:8",
                "networks": [{
                    "network_id": "net_dmz", "ip_address": "172.21.0.30"
                }],
            ...
            },
            {
                ...
                "networks": [{
                    "network_id": "net_dmz", "ip_address": "172.21.0.10"
                }],
                "services": [], "flags": [], "vulnerabilities": []
            }
        ],
        "flags": [
            {
                "id": "flag1", "name": "flag1", "value": "FLAG{1}",
                "points": 10,
                "placement": {
                    "type": "file", "host_id": "host_web",
...
def test_provision_dry_run_operations():
    ...
    last_run = [
        op for op in result.operations if op["type"] == "container.run"
    ][-1]
...
def test_provision_multi_network_host_connects_extras():
    ...
    web_ops = [
        ...
        if op["type"] in ("container.run", "network.connect") and (
            op["args"].get("name") == "host_web" or
            op["args"].get("container") == "host_web"
        )
    ]
